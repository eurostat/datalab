...
To update policies, MinIO has a cronjob that can be triggered manually if needed to automatically syncronise policies with the groups existent in Keycloak. However, if another identity provider is used, the steps to update the policies manually on MinIO are the following:
